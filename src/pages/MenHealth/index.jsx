...
//           errorDetails = details.map(d => d.message).join(', ');
//         }
//       }
...
//                     className="w-full group relative overflow-hidden bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 text-white text-lg font-bold py-5 px-8 rounded-2xl shadow-2xl hover:shadow-3xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
//                     onClick={() => {
//                       // Preview toast
//                       toast('🚀 Analyzing your health data...', {
//                         icon: '🔍',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Extract AI tip from response - check different possible locations
        const aiTip = result.data?.aiTip || result.record?.aiTip || result.aiTip;
        const analysis = result.data?.analysis || result.record?.analysis || result.analysis;
        
        // Store the AI tip to display later
        if (aiTip) {
          setLastAITip(aiTip);
        }

        // Create success toast with AI tip
        const SuccessToast = () => (
          <div className="space-y-3 max-w-md">
            ...
            {analysis?.insights && analysis.insights.length > 0 && (
              <div className="mt-2 p-3 bg-white/10 backdrop-blur-sm rounded-lg border border-white/20">
                <div className="font-semibold text-white mb-1 text-sm">📊 Your Insights:</div>
                <ul className="text-xs text-white/90 space-y-1">
                  {analysis.insights.map((insight, index) => (
                    <li key={index} className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-400"></div>
                      <span>{insight.metric}: <span className="font-semibold">{insight.value}</span></span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        setTimeout(() => {
          toast.custom((t) => (
            <div className={`bg-gradient-to-r from-green-500 to-emerald-600 p-4 rounded-2xl shadow-2xl transform transition-all ${t.visible ? 'animate-slide-in' : 'animate-slide-out'}`}>
              <div className="flex items-center space-x-3">
                <div className="text-2xl">💪</div>
                <div>
                  <div className="font-bold text-white">Strong Work!</div>
                  <div className="text-sm text-white/90">Your health data is now tracked and secured</div>
                </div>
              </div>
            </div>
          ), { duration: 3000 });
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-white/70 rounded-xl">
                      <div className="text-center text-gray-600">
                        ...
                        <div className="text-sm">Submit your health data to receive personalized AI-powered health tips and insights.</div>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 p-3 bg-white/50 rounded-lg">
                        <div className="text-purple-600">💡</div>
                        <div className="text-sm text-gray-700">Personalized health recommendations</div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-white/50 rounded-lg">
                        <div className="text-blue-600">📈</div>
                        <div className="text-sm text-gray-700">Trend analysis and insights</div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-white/50 rounded-lg">
                        <div className="text-green-600">🎯</div>
                        <div className="text-sm text-gray-700">Actionable wellness advice</div>
                      </div>
                    </div>
                  </div>
                )}
