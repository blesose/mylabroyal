...
// const API_BASE_URL =   'https://mylab-lts4.onrender.com/api';
...
class ApiService {
  ...
  async getACommunityPost() {
    const response = await this.client.get('/communitypost/community/posts');
    return response.data;
  }
  ...
  async deleteFitness(id) {
    const response = await this.client.delete(`/fitnessnutrition/fitness/delete-fitness/${id}`);
    return response.data;
  }
  ...
  async deleteSelfCare(id) {
    if (!id) {
      throw new Error("Self-care ID is required for deletion");
    }
    const response = await this.client.delete(`/selfhealth/selfcare/delete-selfcare/${id}`);
    return response.data;
  }

  async fetchAllSelfCareActivities() {
    const response = await this.client.get('/selfhealth/selfcare/fetch-selfcare/');
    return response.data;
  }

  // Lab Insights endpoints
  async createInsight(labinsightsData) {
    const response = await this.client.post('/labinsights/lab/create/', labinsightsData);
    return response.data;
  }
