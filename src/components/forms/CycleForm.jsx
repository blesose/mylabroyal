  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        crampsIntensity: formData.crampsIntensity,
        flowConsistency: formData.flowConsistency
      };

      console.log('📨 Sending data:', dataToSend);

      const result = await apiService.createCycle(dataToSend);
      console.log('✅ Server response:', result);
      
      // Store the full data including prediction
      const fullData = {
        ...formData,
        id: result.data?.cycle?._id || result._id || Date.now(),
        _id: result.data?.cycle?._id || result._id,
        cycleLength: result.data?.cycleLength,
        predictedNext: result.data?.nextCycle
      };
      ...
      // Show success with prediction info
      const predictedDate = result.data?.nextCycle 
        ? new Date(result.data.nextCycle).toLocaleDateString()
        : 'unknown';
      
      toast.custom((t) => (
        <CustomSuccessToast
          message={mode === 'create' ? 'Cycle data saved successfully!' : 'Cycle data updated successfully!'}
          description={`Next cycle predicted: ${predictedDate}`}
        />
      ), {
        duration: 5000,
        position: isMobile ? 'top-center' : 'bottom-right',
      ...
      let errorMessage = 'Failed to save cycle data';
      ...
      if (error.response?.data) {
        ...
        errorDescription = 'Please log in to save cycle data.';
      }
      ...
      <AnimatePresence>
        {predictionData && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="mt-4 sm:mt-6 md:mt-8 bg-gradient-to-r from-purple-50 to-pink-50/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border-2 border-purple-200/80"
          >
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              ...
              <div>
                <h3 className="font-bold text-purple-800 text-sm sm:text-base">Next Cycle Prediction</h3>
                <p className="text-xs sm:text-sm text-purple-600">Based on your cycle patterns</p>
              </div>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="bg-white/60 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4">
                <p className="text-xs sm:text-sm text-purple-600 mb-1">Predicted Start Date</p>
                <p className="text-base sm:text-lg md:text-xl font-bold text-purple-800">
                  {predictionData.predictedNext 
                    ? new Date(predictionData.predictedNext).toLocaleDateString('en-US', {
                        weekday: 'long',
                        month: 'long',
                        day: 'numeric',
                        year: 'numeric'
                      })
                    : 'Add more cycles for prediction'}
                </p>
              </div>
              <div className="bg-white/60 backdrop-blur-sm rounded-lg sm:rounded-xl p-3 sm:p-4">
                <p className="text-xs sm:text-sm text-purple-600 mb-1">Estimated Cycle Length</p>
                <p className="text-base sm:text-lg md:text-xl font-bold text-purple-800">
                  {predictionData.cycleLength ? `${predictionData.cycleLength} days` : '-- days'}
                </p>
              </div>
            </div>
            
            <div className="mt-3 sm:mt-4 text-xs sm:text-sm text-purple-600 flex items-center gap-1 sm:gap-2">
              <Info className="w-3 h-3 sm:w-4 sm:h-4" />
              This is an estimate based on your recorded cycles. Regular tracking improves accuracy.
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
              ...
              <div className="bg-white/60 backdrop-blur-sm p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl">
                ...
                <div className="font-bold text-emerald-800 text-base sm:text-lg md:text-xl capitalize truncate">
                  {savedData.mood?.replace('-', ' ') || 'neutral'}
                </div>
              </div>
            </div>
          </motion.div>
