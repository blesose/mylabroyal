...
//   const { dispatch } = useApp();
//   const [formData, setFormData] = useState({
//     conceptionDate: '', // ADDED: Conception date field
//     week: '',
//     notes: ''
//   });
//   const [selectedSymptoms, setSelectedSymptoms] = useState([]);
...
//             <AlertCircle className="w-6 h-6 text-red-500 mt-0.5" />
//             <div>
//               <p className="font-medium text-gray-800 mb-1">Conception Date Required</p>
//               <p className="text-sm text-gray-600">Please enter your conception date</p>
//             </div>
//           </div>
//         </div>
//       ), {
...
//       // Prepare data matching backend validator requirements
//       const submissionData = {
//         // REQUIRED FIELDS (from validator):
//         userId: user._id, // REQUIRED
//         conceptionDate: new Date(formData.conceptionDate).toISOString(), // REQUIRED
//         week: parseInt(formData.week), // REQUIRED
...
//         // Remove timestamp if not in validator schema
//         // timestamp: new Date().toISOString()
//       };

//       console.log('Sending pregnancy data:', submissionData);
...
//     } catch (error) {
//       console.error('Full pregnancy error details:', {
...
//   const quickWeekSelect = (week) => {
//     // When user selects a week, automatically calculate conception date
//     const today = new Date();
//     const conceptionDate = new Date(today);
//     conceptionDate.setDate(today.getDate() - (week * 7));
    
//     setFormData({ 
//       ...formData, 
//       week: week.toString(),
//       conceptionDate: conceptionDate.toISOString().split('T')[0] // Format as YYYY-MM-DD
//     });
...
//       >
//         <div className="p-4 sm:p-6 md:p-8">
//           {/* Conception Date - ADDED THIS SECTION */}
//           <div className="mb-6 sm:mb-8">
//             <label className="block text-sm sm:text-base font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
//               <Calendar className="w-4 h-4 sm:w-5 sm:h-5" />
//               Conception Date / Last Menstrual Period *
//             </label>
...
//               />
//               <p className="text-xs sm:text-sm text-pink-600 mt-2">
//                 Enter the date of your last menstrual period or estimated conception
//               </p>
//             </div>
//           </div>
...
//           <motion.button
//             type="submit"
//             disabled={isSubmitting || !formData.week || !formData.conceptionDate}
//             whileHover={{ scale: 1.05 }}
//             whileTap={{ scale: 0.95 }}
//             className="w-full bg-gradient-to-r from-pink-500 to-purple-500 text-white py-4 rounded-xl font-semibold hover:shadow-xl hover:shadow-pink-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
//           >
...
//           initial={{ opacity: 0, y: 20 }}
//           animate={{ opacity: 1, y: 0 }}
//           className="mt-4 sm:mt-6 md:mt-8 bg-gradient-to-r from-green-50/80 to-emerald-50/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border-2 border-emerald-200/80"
//         >
//           <div className="flex items-start gap-3 sm:gap-4">
//             <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-gradient-to-r from-emerald-400 to-green-500 flex items-center justify-center flex-shrink-0">
//               <Calendar className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
//             </div>
//             <div>
//               <h3 className="text-sm sm:text-base md:text-lg font-bold text-emerald-800 mb-2 sm:mb-3">
//                 Estimated Due Date
//               </h3>
//               <p className="text-xs sm:text-sm text-emerald-700/80 mb-3">
//                 {(() => {
//                   const conceptionDate = new Date(formData.conceptionDate);
...
//                 Based on conception date: {new Date(formData.conceptionDate).toLocaleDateString()}
//               </div>
//             </div>
//           </div>
//         </motion.div>
...
const PregnancyForm = () => {
  ...
  const [calculatedCurrentWeek, setCalculatedCurrentWeek] = useState(null);
  ...
  const calculateDueDate = (conceptionDate) => {
    ...
    date.setDate(date.getDate() + 280); // 40 weeks * 7 days
    return date;
  };

  // Calculate current week from conception date
  const calculateCurrentWeek = (conceptionDate) => {
    const today = new Date();
    const conception = new Date(conceptionDate);
    const diffTime = Math.abs(today - conception);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return Math.min(42, Math.max(1, Math.floor(diffDays / 7)));
  };

  // Update calculations when conception date changes
  const updateCalculations = (conceptionDate) => {
    if (conceptionDate) {
      ...
      // Auto-update week field with calculated current week
      if (!formData.week) {
        setFormData(prev => ({ ...prev, week: currentWeek.toString() }));
      }
      
      toast.success(`Calculated: Week ${currentWeek}, Due ${dueDate.toLocaleDateString()}`, {
        duration: 3000,
        position: 'bottom-right',
        icon: '📅',
      });
  ...
  const handleSubmit = async (e) => {
    ...
    const currentWeek = formData.week ? parseInt(formData.week) : calculatedCurrentWeek;
    ...
    try {
      ...
      // Prepare data matching backend model requirements
      const submissionData = {
      ...
      toast.custom((t) => (
        <CustomSuccessToast
          message={`Week ${currentWeek} Progress Saved!`}
  ...
  const quickWeekSelect = (week) => {
    // When user selects a week, calculate conception date from today
    const today = new Date();
    const conceptionDate = new Date(today);
    conceptionDate.setDate(today.getDate() - (week * 7));
    ...
    // Update calculations
    updateCalculations(conceptionDate);
  ...
  // Handle conception date change
  const handleConceptionDateChange = (date) => {
    setFormData({ ...formData, conceptionDate: date });
    if (date) {
    ...
    <div className="max-w-2xl mx-auto px-3 sm:px-4 md:px-6">
      ...
      >
        <div className="p-4 sm:p-6 md:p-8">
          ...
          {calculatedDueDate && calculatedCurrentWeek && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-6 sm:mb-8 bg-gradient-to-r from-green-50/80 to-emerald-50/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-5 border-2 border-emerald-200/80"
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <h4 className="text-sm font-semibold text-emerald-800 mb-2">Current Week</h4>
                  <div className="text-2xl font-bold text-emerald-700">
                    Week {calculatedCurrentWeek}
                  </div>
                  <p className="text-xs text-emerald-600 mt-1">
                    {calculatedCurrentWeek <= 13 ? 'First Trimester' : 
                     calculatedCurrentWeek <= 27 ? 'Second Trimester' : 'Third Trimester'}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-emerald-800 mb-2">Estimated Due Date</h4>
                  <div className="text-lg font-bold text-emerald-700">
                    {calculatedDueDate.toLocaleDateString('en-US', { 
                      month: 'short', 
                      day: 'numeric', 
                      year: 'numeric' 
                    })}
                  ...
                  <p className="text-xs text-emerald-600 mt-1">
                    {Math.max(0, 280 - (calculatedCurrentWeek * 7))} days remaining
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          {/* Week Input (can override calculated) */}
          <div className="mb-6 sm:mb-8">
            <label className="block text-sm sm:text-base font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
              <Calendar className="w-4 h-4 sm:w-5 sm:h-5" />
              Current Pregnancy Week (Optional - Override)
            </label>
            ...
            <div className="mb-4 sm:mb-6">
              <input
                ...
                onChange={(e) => setFormData({ ...formData, week: e.target.value })}
                className="w-full px-4 sm:px-5 py-3 sm:py-4 border-2 border-pink-300 rounded-lg sm:rounded-xl focus:ring-2 sm:focus:ring-4 focus:ring-pink-500/20 focus:border-pink-500 transition-all bg-white/80 text-pink-700 text-base sm:text-lg"
                placeholder={`Calculated: Week ${calculatedCurrentWeek || '...'}`}
              />
              <p className="text-xs sm:text-sm text-pink-600 mt-2">
                Leave empty to use calculated week, or enter your own
              </p>
            </div>
        ...
        <div className="p-4 sm:p-6 md:p-8 border-t border-gray-200/80 bg-gradient-to-r from-gray-50/50 to-white/50">
          <motion.button
            type="submit"
            disabled={isSubmitting || !formData.conceptionDate}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="w-full bg-gradient-to-r from-pink-500 to-purple-500 text-white py-4 rounded-xl font-semibold hover:shadow-xl hover:shadow-pink-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
          >
      ...
      {/* Milestone Tracker */}
      {calculatedCurrentWeek && weekMilestones[calculatedCurrentWeek] && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mt-4 sm:mt-6 md:mt-8 bg-gradient-to-r from-yellow-50/80 to-amber-50/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border-2 border-amber-200/80"
        >
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div>
              <h3 className="text-sm sm:text-base md:text-lg font-bold text-amber-800 mb-2 sm:mb-3">
                Week {calculatedCurrentWeek} Milestone
              </h3>
              <p className="text-xs sm:text-sm text-amber-700/80">
                {weekMilestones[calculatedCurrentWeek]}
              </p>
              <div className="mt-3">
                <div className="w-full h-2 bg-amber-200 rounded-full overflow-hidden">
                  <motion.div
                    className="h-full bg-gradient-to-r from-amber-400 to-orange-500"
                    initial={{ width: '0%' }}
                    animate={{ width: `${(calculatedCurrentWeek / 40) * 100}%` }}
                    transition={{ duration: 1 }}
                  />
                </div>
                <div className="flex justify-between mt-1 text-xs text-amber-600">
